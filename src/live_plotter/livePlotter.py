...
class plotter():
    ...
    def __init__(self):
        ...
        rospy.Subscriber('SIG', imu, self.Sig_callback, queue_size=10)
    ...
    def plot_data(self, i):
        ...
        if self.Plot_Ellipses:
            ...
            self.ax1.plot(self.ellipse[0], self.ellipse[1], color='c')
    ...
    def Sig_callback(self, msg):
        if self.Plot_Ellipses:
            P = np.reshape(msg.orientation_covariance,(3,3))
            print("P:", P)
            u, s, v = np.linalg.svd(P[0:2, 0:2])
            c = u * 2 *np.sqrt(s)
            self.ellipse = c @ self.cir_points
            self.ellipse[0]+=self.Mu_x
            self.ellipse[1]+=self.Mu_y
